...
class Migration(DataMigration):
    ...
    depends_on = (
        ('tags', '0003_main_article_category'),
    )

    def forwards(self, orm):
        for article in orm['articles.Article'].objects.all():
            ct = orm['contenttypes.contenttype'].objects.get(model='article')
            taggeditems = orm['taggit.taggeditem'].objects.filter(content_type=ct, object_id=article.id)
            if taggeditems:
                for taggeditem in taggeditems:
                    if taggeditem.tag.name.isupper():
                        tag, created = orm['tags.ModTags'].objects.get_or_create(name=taggeditem.tag.name)
                        if created:
                            tag.save()
                        article.mod_tags.add(tag)

                    if taggeditem.tag.name.islower():
                        tag, created = orm['tags.UserTags'].objects.get_or_create(name=taggeditem.tag.name)
                        tag.main_article_category = False
                        if created:
                            tag.save()
                        article.user_tags.add(tag)
                    article.save()

    def backwards(self, orm):
        for article in orm['articles.Article'].objects.all():
            article.user_tags.clear()
            article.mod_tags.clear()
            article.save()
